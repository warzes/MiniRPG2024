...
{
	const uint64_t data_size = size.width * size.height * size.depthOrArrayLayers * channels * sizeof(uint8_t);

	wgpu::ImageCopyTexture ict{};
	ict.texture = texture;
	ict.aspect = wgpu::TextureAspect::All;

	wgpu::TextureDataLayout tdl{};
	tdl.offset = 0;
	tdl.bytesPerRow = size.width * channels * sizeof(uint8_t);
	tdl.rowsPerImage = size.height;

	device.GetQueue().WriteTexture(&ict, pixels, data_size, &tdl, &size);
}
...
{
	wgpu_mipmap_generator* mipmap_generator = new wgpu_mipmap_generator();
	...
	mipmap_generator->sampler = device.CreateSampler(&sampler_desc);
